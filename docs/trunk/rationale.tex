...
the same.  The basic notation is borrowed from C-like languages, the Unix
shell and inspired by Irssi's configuration files and CSS notation.  This
should make it easy to read for experienced programmers.  Beginners should not
...
\subsubsection{Rejected idea}

Originally, we decided a statement should always be ended by a
semi-colon and strings should be enclosed in quotes, like in C, for
maximum clarity and ease of parsing.

This would also allow the user to make statements span multiple lines
in the file and write one-liners with ease, providing maximum flexibility
of notation.

\subsubsection{Accepted idea}

The above idea is more formal and is thus probably better design.
But it turned out to be extremely inpractical in Turner's
configuration files, where one is constantly writing very simple lines.
In that case, it became a real bother to wite things like
\verb|! "echo \"Hello, World\"";|.  Instead, it became increasingly
clear to us that the right way would be to abandon the extremely
academic and formal notation and adopt a more loose style of notation.
Using a different notation exclusively for lines with pipes is
unacceptable, since it breaks consistency, so the change was
incorporated in every file format.

Now, statements are until the end of a line, and if one wishes to
continue on a new line, a single backslash at the end of the line
is needed, like in the shell or in Makefiles.  If for some reason it is
necessary to be precise about spaces, one can still enclose the string in
double quotes, again like in the shell or a Makefile.
