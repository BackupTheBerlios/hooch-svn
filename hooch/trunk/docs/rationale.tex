...
Because it is inpractical to specify many options common to each contact
every time a new contact is defined, there should be a way to define
default options applicable to all contacts.
We considered defining a special contact called `default', but this is
not a good idea because this contact should not contain identities or an
address field.
The `cascade' idea of properties which fall down from the default to a
contact to an identity does not apply here either.
